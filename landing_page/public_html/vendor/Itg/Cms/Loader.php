...
class Loader
{
    ...
    public function __construct()
    {
        ...
        Hooks::addListener('APP_POST_ROUTING', function()
        {
            ...
            if (Auth::isLoggedIn() AND Me::isAdmin())
            {
                ...
                Nav::build()->group('Data', 'fa-cogs')->item('Submissions', 'fa-users')->target(PageController::class, 'csv')->add();
            }
    ...
    public function _routes()
    {
        ...
        $routes->set('/admin/csv/',
            [
                'controller' => PageController::class,
                'action'     => 'csv'
            ]);

        $routes->set('/admin/attendees/',
            [
                'controller' => PageController::class,
                'action'     => 'attendees'
            ]);

        $routes->set('/admin/nonattendees/',
            [
                'controller' => PageController::class,
                'action'     => 'nonattendees'
            ]);
