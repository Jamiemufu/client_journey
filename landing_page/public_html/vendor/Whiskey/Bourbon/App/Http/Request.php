...
class Request
{
     ...
     */
    public function __construct(Headers $headers, Cookie $cookie, SafeString $safe_string)
    {
        ...
        $cookies                       = $this->_getCookies($cookie);
        $this->_properties->cookies    = $this->_sanitiseArray($cookies);
    ...
    /**
     * Get cookies, taking into account the framework's cookie handler
     * @param  Cookie $cookie Cookie object
     * @return array          Array of cookies
     */
    protected function _getCookies(Cookie $cookie)
    {
        ...
        foreach ($_COOKIE as $key => $value)
        {
            ...
            /*
             * Cookies set by the framework
             */
            if ($cookie->isValid($key))
            {
                $result[$key] = $cookie->read($key);
            }
